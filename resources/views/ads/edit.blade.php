    ...
    <div class="col-lg-8">
        ...
        <form method="POST" action="{{ route('ads.update', $ad) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="">حدد البلد</label>
                <select class="form-control" name="country_id">
                    @foreach ($countries as $item)
                        <option value="{{ $item->id }}" {{ $ad->country_id == $item->id ? 'selected' : '' }}>
                            {{ $item->name }} </option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-control" name="category_id">
                    @foreach ($categories as $item)
                        <option value="{{ $item->id }}" {{ $ad->category_id == $item->id ? 'selected' : '' }}>
                            {{ $item->name }} </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="row">
                    <div class="col-lg-7">
                        <input type="number" class="form-control" value="{{ $ad->price }}" name="price" step="any"
                            placeholder="" title="السعر">
                    </div>
